...
- The project uses a HyFormer-style backbone with:
  - non-sequential features mapped into `num_non_seq_tokens` feature tokens
  - each behavioral sequence mapped into its own sequence-token stream
  - dedicated query/global tokens generated for each sequence branch from non-sequential features and sequence pooling summaries
  - stacked `Query Decoding -> Query Boosting` layers
- Query Generation:
  - initial per-sequence queries are generated from concatenated non-sequential features and per-sequence pooled summaries
...
- Query Boosting:
  - concatenates decoded per-sequence queries with non-sequence tokens
  - applies token mixing plus a per-token FFN
  - returns updated per-sequence query and non-sequence tokens
