...
SEQ_ENCODER_CHOICES = ("longer", "full_transformer", "swiglu")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download TAAC2026/data_sample_1000, convert it into HyFormer inputs, and run a small classifier."
    )
    ...
    parser.add_argument("--num-sequences", type=int, default=3, help="How many grouped sequence branches HyFormer should use.")
    parser.add_argument("--global-tokens-per-seq", type=int, default=1, help="How many query/global tokens each sequence branch owns.")
    parser.add_argument("--num-non-seq-tokens", type=int, default=13, help="Number of non-sequential feature tokens used by Query Boosting.")
    ...
    parser.add_argument(
        "--seq-encoder-type",
        choices=SEQ_ENCODER_CHOICES,
        default="longer",
        help="Sequence encoder inside each HyFormer layer: longer, full_transformer, or swiglu.",
    )
    parser.add_argument("--short-seq-len", type=int, default=8, help="Compressed sequence length used by the longer encoder.")
...
def main() -> None:
    ...
    non_seq_x, seq_x, labels, metadata = build_tensors(rows, args.seq_len, args.num_sequences)
    ...
    print(
        f"[run] non_seq={tuple(non_seq_x.shape)} seq={tuple(seq_x.shape)} "
        f"classes={int(labels.max().item()) + 1} grouped_sequences={metadata['num_sequences']}"
    )
    print(
        f"[run] amp={use_amp} amp_dtype={args.amp_dtype} grad_scaler={scaler.is_enabled()} "
        f"device_type={device_type} seq_encoder_type={args.seq_encoder_type}"
    )
    ...
    save_run_artifacts(args.output_dir, metadata, args_payload, last_checkpoint_state, args.save_checkpoint)
    if best_model_state is not None and args.save_checkpoint:
        best_checkpoint_path = args.output_dir / f"best_model_epoch{best_epoch:02d}_auc{best_val_auc:.4f}.pt"
        torch.save(best_checkpoint_state, best_checkpoint_path)
        print(f"[run] best checkpoint saved to {best_checkpoint_path}")
